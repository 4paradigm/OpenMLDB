...
| [`FROM Clause`](#tablerefs)                    | **``✓``**    | **``✓``**           | **``✓``**           | The From clause indicates the data source.<br />The data source can be one table (`select * from t;`) or multiple tables that JOIN together (`select * from t1 join t2;`) or no table ( `select 1+1;`), see [NO_TABLE SELECT](../dql/NO_TABLE_SELECT_CLAUSE.md)                                                                                                                         |
