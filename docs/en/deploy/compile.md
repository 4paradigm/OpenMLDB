...
Keep in mind that you should always use the same version of both compile image and [OpenMLDB version](https://github.com/4paradigm/OpenMLDB/releases). This section demonstrates compiling for [OpenMLDB v0.7.1](https://github.com/4paradigm/OpenMLDB/releases/tag/v0.7.1) under `hybridsql:0.7.1` ，If you prefer to compile on the latest code in `main` branch, pull `hybridsql:latest` image instead.
...
3. Download the OpenMLDB source code inside the docker container, and setting the branch into v0.7.1

   ```bash
   cd ~
   git clone -b v0.7.1 https://github.com/4paradigm/OpenMLDB.git
   ```
