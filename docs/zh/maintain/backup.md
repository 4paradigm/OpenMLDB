...
- 在最佳实践配置下，高可用的机制为：
  ...
  - 当特定分片对应的三个 tablets 如果有一个下线，则服务不收影响，并且下线的 tablet 可以自动恢复。但是在数据完整性上（1）如果线下的是从分片，则数据完整性不受影响；（2）如果下线的是主分片并且有写流量，则少量数据可能会丢失（部分写入数据可能未及时同步到从分片）
  - 当对应的三个 tablets 如果有两个下线：(1) 如果都是从分片，则数据和服务均不受影响，并且下线的 tablet 可以自动恢复；(2) 如果其中有一个主分片，则数据完整性会收到影响，但是依然可以持续提供服务。
  - **总结来说，（1）当一个 tablet 下线，服务不受影响，但如果下线时有写流量，则有可能造成小部分未及时同步的数据丢失；（2）如果同一时间两个 tablet 下线，则服务不受影响，但是数据完整性一般无法保证（一个 tablet 普遍存储了多个分片）；（3）如果多于两个 tablet 下线，则服务和数据完整性均无法保证。**
