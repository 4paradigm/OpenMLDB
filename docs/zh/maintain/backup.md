...
- OpenMLDB 的高可用机制是以数据分片为最小单位。在最佳实践配置下，每一个分片的有三个副本，必定分布在三个不同的 tablets 上，其中有一个主分片（leader），两个为从分片（followers）。则当该分片对应的 tablets 离线时，不同的 tablets 离线数目，对该分片的高可用会造成不同的影响，如下表格总结：

| 该分片对应的 tablet 离线数目 | 服务可用性 | 该分片的数据完整性                                                   | 离线 tablet 自动恢复 |
| ---------------------------- | ---------- | ------------------------------------------------------------ | -------------------- |
