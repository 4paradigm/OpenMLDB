...
### 为什么日志中有Fail to write into Socket的警告日志？
```
...
### Last Join right table is empty 警告日志
通常来讲，这是一个正常现象，不代表集群异常。只是runner中join右表为空，是可能的现象，大概率是数据问题。

## Client FAQ

### 为什么收到Reached timeout的警告日志？
```
...
1. CLI: 目前无法更改
2. JAVA: sdk直连，调整`SdkOption.requestTimeout`; JDBC，调整url中的参数`requestTimeout`
3. Python: 目前无法更改

### 为什么收到Got EOF of Socket的警告日志？
```
