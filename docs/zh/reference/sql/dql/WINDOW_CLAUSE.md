...
*窗口调用函数*实现了类似于聚合函数的功能。 不同的是，窗口调用函数不需要将查询结果打包成一行输出———在查询输出中，每一行都是分开的。 然而，窗口调用函数可以扫描所有的行，根据窗口调用函数的分组规范(`PARTITION BY`列)， 这些行可能会是当前行所在组的一部分。如下展示了4肿窗口调用函数的语法：
...
```sql
SELECT select_expr [, select_expr ...], window_function_name(expr) OVER window_name, ... 
FROM ... 
WINDOW AS window_name (PARTITION BY ... ORDER BY ... ROWS BETWEEN ... AND ...)
...
SELECT select_expr [,select_expr...], window_function_name(expr) OVER window_name, ... 
FROM ... 
WINDOW AS window_name (PARTITION BY ... ORDER BY ... ROWS_RANEG BETWEEN ... AND ...)
```
...
`PARTITION BY`选项将查询的行分组进入*partitions*， 这些行在窗口函数中单独处理。`PARTITION BY`和查询级别`GROUP BY` 子句做相似的工作，但是它的表达式只能作为表达式，不能作为输出列的名字或数。OpenMLDB要求必须配置`PARTITION BY`。并且目前**仅支持按列分组**，无法支持按运算和函数表达式分组。

`ORDER BY` 选项决定分区中的行被窗口函数处理的顺序。它和查询级别`ORDER BY`子句做相似的工作， 但是它也不能作为输出列的名字或数。同样，OpenMLDB要求必须配置`ORDER BY`。并且目前**仅支持按列排序**，无法支持按运算和函数表达式排序。
...
SQL标准的RANGE类窗口OpenMLDB系统目前暂不支持。它们的差异如下图所示

![Figure 1: window frame type](../dql/images/window_frame_type.png)

- ROWS: 数据按行划入窗口，根据条数滑出窗口
- ROWS_RANGE：数据按行划入窗口，根据时间区间滑出窗口
- RANGE: 数据按时间粒度划入窗口（一次可能滑入多条同一时刻的数据行），按时间区间滑出窗口
...
- OpenMLDB默认边界是闭合的。但支持以OPEN关键字来设置边界为开区间。
:::{note}
请注意：标准SQL中，还支持FOLLOWING的边界，当前OpenMLDB暂不支持。
- :::
#### **Example: 有名窗口（Named Window）**

```SQL
SELECT sum(col2) 
OVER w1 as w1_col2_sum 
FROM t1
...
```SQL
SELECT id, pk1, col1, std_ts,sum(col1) 
OVER (PARTITION BY pk1 ORDER BY std_ts ROWS BETWEEN 1 PRECEDING AND CURRENT ROW) as w1_col1_sum
from t1;
```
...
窗口范围为前1000行到当前行, 除了current row以外窗口内不包含当前时刻的其他数据
```SQL
SELECT sum(col2) OVER w1 as w1_col2_sum FROM t1
...
窗口范围为前10s到当前行，除current row以外窗口内不包含当前时刻的其他数据
```SQL
SELECT sum(col2) OVER w1 as w1_col2_sum FROM t1
...
OpenMLDB允来限定窗口内数据条数。具体来说，可以在窗口定义里使用**MAXSIZE**关键字，来限制有效窗口内最大数据条数。
...
窗口范围为前10s的数据到当前条，同时限制窗口内条数不超过3条
```sql
SELECT sum(col2) OVER w1 as w1_col2_sum FROM t1
