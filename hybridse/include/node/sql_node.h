...
class DynamicUdafFnDefNode : public FnDefNode {
 public:
    DynamicUdafFnDefNode(const std::string &name, const std::vector<const TypeNode *> &arg_types,
        ...
        : FnDefNode(kDynamicUdafFnDef),
          name_(name),
          arg_types_(arg_types),
          init_context_node_(init_context_node),
    ...
    const TypeNode *GetStateType() const {
        if (init_node_ != nullptr) {
            return init_node_->GetReturnType();
        } else {
            return GetElementType(0);
        }
    }
    ...
    const TypeNode *GetReturnType() const override {
        if (output_node_ != nullptr) {
            return output_node_->GetReturnType();
        } else {
            return GetStateType();
        }
    }
    ...
    bool IsReturnNullable() const override {
        if (output_node_ != nullptr) {
            return output_node_->IsReturnNullable();
        } else if (init_node_ != nullptr) {
            return init_node_->IsReturnNullable() || update_node_->IsReturnNullable();
        } else {
            return true;
        }
    }
    ...
    DynamicUdafFnDefNode *ShadowCopy(NodeManager *) const override;
    DynamicUdafFnDefNode *DeepCopy(NodeManager *) const override;
