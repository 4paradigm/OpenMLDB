    ...
    */
    void write_cbor(const BasicJsonType& j) {
        switch (j.type()) {
            ...
            case value_t::boolean: {
                oa->write_character(j.m_value.boolean ? to_char_type(0xF5) : to_char_type(0xF4));
                break;
            }
            ...
            case value_t::string: {
                ...
                oa->write_characters(reinterpret_cast<const CharType*>(j.m_value.string->c_str()),
    ...
    */
    void write_msgpack(const BasicJsonType& j) {
        switch (j.type()) {
            ...
            {
                oa->write_character(j.m_value.boolean ? to_char_type(0xC3) : to_char_type(0xC2));
                break;
            }
    ...
    */
    void write_ubjson(const BasicJsonType& j, const bool use_count, const bool use_type, const bool add_prefix = true) {
        switch (j.type()) {
            ...
            case value_t::boolean: {
                if (add_prefix) {
                    oa->write_character(j.m_value.boolean ? to_char_type('T') : to_char_type('F'));
                }
                break;
            }
            ...
            case value_t::array: {
                ...
                if (use_type && !j.m_value.array->empty()) {
                    ...
                    const bool same_prefix = std::all_of(
                        j.begin() + 1, j.end(),
                        [this, first_prefix](const BasicJsonType& v) { return ubjson_prefix(v) == first_prefix; });
            ...
            case value_t::binary: {
                ...
                if (use_type) {
                    oa->write_characters(reinterpret_cast<const CharType*>(j.m_value.binary->data()),
    ...
    void write_bson_unsigned(const string_t& name, const BasicJsonType& j) {
        if (j.m_value.number_unsigned <= static_cast<std::uint64_t>((std::numeric_limits<std::int32_t>::max)())) {
            ...
            JSON_THROW(out_of_range::create(407,
                                            concat("integer number ", std::to_string(j.m_value.number_unsigned),
