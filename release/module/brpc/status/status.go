...
func (m *MetricSet) Fetch() ([]common.MapStr, error) {
	...
	var endpoint string
	if m.port == "" {
		filename := "conf/tablet.flags"
		conf, err = GetConf(filename)
		if err != nil {
			fmt.Println(err)
			return nil, err
		}
		confinfo := strings.Split(string(conf), "\n")
		for i := 0; i < len(confinfo); i++ {
			if strings.Contains(confinfo[i], "endpoint") {
				targetip := strings.Split(confinfo[i], "=")
				if len(targetip) == 2 {
					endpoint = targetip[1]
				}
			}
