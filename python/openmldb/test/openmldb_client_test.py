...
class TestOpenMLDBClient(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        # anhui 2
        insert2 = "insert into tsql1010 values(1001, '2020-12-26', 'hefei', ?, ?);"
        # fujian fuzhou
        insert3 = "insert into tsql1010 values(1002, '2020-12-27', ?, ?, 3);"
        # 1003 2020-11-28 jiangxi nanchang 4
        insert4 = "insert into tsql1010 values(?, ?, ?, ?, ?);"
        # 2020-11-29
        insert5 = "insert into tsql1010 values(1004, ?, 'hubei', 'wuhan', 5);"
        connection.execute(insert1)
        connection.execute(insert2, ({"col4": "anhui", "col5": 2}))
        connection.execute(insert3, ({"col3": "fujian", "col4": "fuzhou"}))
        connection.execute(insert4, ({
                           "col1": 1003, "col2": "2020-12-28", "col3": "jiangxi", "col4": "nanchang", "col5": 4}))
        ...
        # test request mode
        rs = connection.execute("select * from tsql1010;", ({
                                "col1": 9999, "col2": '2020-12-27', "col3": 'zhejiang', "col4": 'hangzhou', "col5": 100}))
        ...
        logging.info(
            "[Execute]: select * from tsql1010 where col3 = ?; ('hefei')")
        rs = connection.execute(
            "select * from tsql1010 where col3 = ?;", ('hefei'))
        rs = list(rs)
        expectRows = [
            (1001, '2020-12-26', 'hefei', 'anhui', 2),
        ]
        ...
        time.sleep(2)
        connection.execute(
            "create procedure sp (col1 bigint, col2 date, col3 string, col4 string, col5 int) begin select * from tsql1010; end;")
        raw_connection = engine.raw_connection()
        mouse = raw_connection.cursor()
        rs = mouse.callproc(
            "sp", ({"col1": 1002, "col2": '2020-12-27', "col3": 'fujian', "col4": 'fuzhou', "col5": 3}))
        logging.info(
            "[Execute]: create procedure sp (col1 bigint, col2 date, col3 string, col4 string, col5 int) begin select * from tsql1010; end;")
        ...
        rs = mouse2.batch_row_request("select * from tsql1010;", (), ({
                                      "col1": 1002, "col2": '2020-12-27', "col3": 'fujian', "col4": 'fuzhou', "col5": 3}))
        for i in range(rs.rowcount):
            i = rs.fetchone()
            logging.info(i)
            if i == None:
        ...
        rs = mouse3.batch_row_request("select * from tsql1010;", (), ({"col1": 1002, "col2": '2020-12-27', "col3": 'fujian', "col4": 'fuzhou', "col5": 3}, {
                                      "col1": 1003, "col2": "2020-12-28", "col3": "jiangxi", "col4": "nanchang", "col5": 4}))
        for i in range(rs.rowcount):
            i = rs.fetchone()
            logging.info(i)
    ...
    def check_result(self, rs, expectRows, orderIdx=0):
        ...
        for row in rs:
            self.assertEqual(
                row, expectRows[i], "not equal row: {}\n{}".format(row, expectRows[i]))
            i += 1
    ...
    def check_fetchmany(self, connection):
        ...
        self.assertTrue(result.fetchmany() == [
                        (1002, '2020-12-27', 'fujian', 'fuzhou', 3)])
        self.assertTrue(result.fetchmany(size=2) == [
                        (1001, '2020-12-26', 'hefei', 'anhui', 2), (1000, '2020-12-25', 'guangdon', '广州', 1)])
        self.assertTrue(result.fetchmany(size=4) == [
                        (1004, '2020-12-29', 'hubei', 'wuhan', 5), (1003, '2020-12-28', 'jiangxi', 'nanchang', 4)])
    ...
    def test_parameterized_query(self):
        ...
        logging.info(
            "[Execute]: select * from tsql1010 where col3 = ?; ('province1')")
        rs = connection.execute(
            "select * from tsql1010 where col3 = ?;", ('province1'))
        rs = list(rs)
        expectRows = [
        ...
        logging.info(
            "[Execute]: select * from tsql1010 where col3 = ?; ('province2')")
        rs = connection.execute(
            "select * from tsql1010 where col3 = ?;", ('province2'))
        rs = list(rs)
        expectRows = [
        ...
        logging.info(
            "[Execute]: select * from tsql1010 where col3 = ?; ('province3')")
        rs = connection.execute(
            "select * from tsql1010 where col3 = ?;", ('province3'))
        rs = list(rs)
        expectRows = [
        ...
        # test parameterized query in batch mode case 3
        logging.info(
            "[Execute]: select * from tsql1010 where col3 = ? and col1 < ?; ('province3', 1008)")
        rs = connection.execute(
            "select * from tsql1010 where col3 = ? and col1 < ?;", ('province3', 1008))
        rs = list(rs)
        expectRows = [
            (1006, '2020-12-31', 'province3', 'city7', 7, 1590738996000),
            (1007, '2021-01-01', 'province3', 'city8', 8, 1590738997000),
        ...
        logging.info(
            "[Execute]: select * from tsql1010 where col3 = ? and col1 < ? and col2 < ?; ('province3', 1008, date.fromisoformat('2021-01-01'))")
        rs = connection.execute("select * from tsql1010 where col3 = ? and col1 < ? and col2 < ?;",
                                ('province3', 1008, date.fromisoformat('2021-01-01')))
        rs = list(rs)
        expectRows = [
            (1006, '2020-12-31', 'province3', 'city7', 7, 1590738996000),
        ]
        ...
        logging.info(
            "[Execute]: select * from tsql1010 where col3 = ? and col6 < ?; ('province3', datetime.fromtimestamp(1590739000000))")
        rs = connection.execute("select * from tsql1010 where col3 = ? and col6 < ?;",
                                ('province3', datetime.fromtimestamp(1590738999.000)))
        rs = list(rs)
        expectRows = [
            (1006, '2020-12-31', 'province3', 'city7', 7, 1590738996000),
            (1007, '2021-01-01', 'province3', 'city8', 8, 1590738997000),
