...
TEST_F(PlannerTest, SimpleConstExprPlanTest) {
    node::NodePointVector list;
    base::Status status;
    int ret = parser_->parse("SELECT 1, 1.0, 1 + 1.0;", list, manager_, status);
    ASSERT_EQ(0, ret);
    ASSERT_EQ(1u, list.size());

    Planner *planner_ptr = new SimplePlanner(manager_);
    node::PlanNodeList plan_trees;
    int code = planner_ptr->CreatePlanTree(list, plan_trees, status);
    if (code != 0) {
        LOG(INFO) << status.msg;
    }
    ASSERT_EQ(0, code);
        ...
        ((node::ProjectListPlanNode *)plan_vec.at(0))->GetProjects().size());
    {
        ...
        ASSERT_EQ(node::kExprPrimary,project->GetExpression()->GetExprType());
        node::ConstNode * node_ptr = dynamic_cast<node::ConstNode*>(project->GetExpression());
        ASSERT_EQ(node::kTypeInt32, node_ptr->GetDataType());
    ...
    }
    {
        ...
        ASSERT_EQ(node::kExprPrimary,project->GetExpression()->GetExprType());
        node::ConstNode * node_ptr = dynamic_cast<node::ConstNode*>(project->GetExpression());
        ASSERT_EQ(node::kTypeDouble, node_ptr->GetDataType());
        ASSERT_EQ(1.0, node_ptr->GetDouble());
    }
    {
        node::ProjectPlanNode *project = dynamic_cast<node::ProjectPlanNode *>(
            ((node::ProjectListPlanNode *)plan_vec.at(0))->GetProjects()[2]);
        ASSERT_EQ(2u, project->GetPos());
        ASSERT_EQ(node::kExprBinary,project->GetExpression()->GetExprType());
        node::BinaryExpr * binary_expr_ptr =dynamic_cast<node::BinaryExpr*>(project->GetExpression());
        {
            node::ConstNode * node_ptr = dynamic_cast<node::ConstNode*>(binary_expr_ptr->children[0]);
            ASSERT_EQ(node::kTypeInt32, node_ptr->GetDataType());
            ASSERT_EQ(1, node_ptr->GetInt());
        }
        {
            node::ConstNode * node_ptr = dynamic_cast<node::ConstNode*>(binary_expr_ptr->children[1]);
            ASSERT_EQ(node::kTypeDouble, node_ptr->GetDataType());
            ASSERT_EQ(1.0, node_ptr->GetDouble());
        }
