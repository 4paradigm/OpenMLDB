...
TEST_F(PlannerTest, SimplePlannerCreatePlanTest) {
    ...
    node::LimitPlanNode
        *limit_node = (node::LimitPlanNode *) select_ptr->GetChildren()[0];
    ...
    ASSERT_EQ(3, ((node::ProjectListPlanNode *) plan_vec.at(0))
        ->GetProjects().size());
...
TEST_F(PlannerTest, SimplePlannerCreatePlanWithWindowProjectTest) {
    ...
    ASSERT_EQ(node::kProjectList, plan_vec.at(0)->GetType());
    ASSERT_EQ(3, ((node::ProjectListPlanNode *) plan_vec.at(0))->GetProjects()
        .size());
    ASSERT_EQ(node::kProjectList, plan_vec.at(1)->GetType());
    ASSERT_EQ(1, ((node::ProjectListPlanNode *) plan_vec.at(1))->GetProjects()
        .size());
    delete planner_ptr;
...
TEST_F(PlannerTest, CreateStmtPlanTest) {
    const std::string sql_str = "create table IF NOT EXISTS test(\n"
