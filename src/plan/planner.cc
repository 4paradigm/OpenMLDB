...
int Planner::CreateSelectPlan(node::SQLNode *select_tree, PlanNode *plan_tree) {
  ...
  if (false == select_expr_list.empty()) {
    for (auto expr : select_expr_list) {
      ...
      int ret = CreateProjectPlanNode(expr, table_node_ptr->GetOrgTableName(),
                                      project_node_ptr);
      if (0 != ret) {
...
int Planner::CreateProjectPlanNode(SQLNode *root, std::string table_name,
...
int SimplePlanner::CreatePlanTree(const NodePointVector &parser_trees,
  ...
  for (auto parser_tree : parser_trees) {
    switch (parser_tree->GetType()) {
      ...
      case node::kCreateStmt: {
        ...
        int ret = CreateCreateTablePlan(parser_tree,
    ...
    switch (column_desc->GetType()) {
      case node::kColumnDesc: {
        ...
        switch (column_def->GetColumnType()) {
          ...
          case node::kTypeString:
            ...
            break;
          default: {
            LOG(WARNING) << "CREATE error: column type "
      ...
      case node::kColumnIndex: {
        ...
        if (column_index->GetName().empty()) {
          column_index->SetName(GenerateName("INDEX", table->indexes_size()));
        }
        if (index_names.find(column_index->GetName()) != index_names.end()) {
          LOG(WARNING) << "CREATE error: INDEX NAME " << column_index->GetName()
                       << " duplicate";
