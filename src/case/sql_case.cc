  ...
  if (isGenerateIndex) {
    for (int i = 0; i < table.indexes_size(); i++) {
      ...
      if (index.ttl_size() > 0) {
        ...
        default: { LOG(WARNING) << "Unrecognized ttl type"; }
        }
      }
...
bool SQLCase::BuildInsertSQLFromMultipleRows(const type::TableDef &table,
  ...
  for (auto row_str = data_line.begin(); row_str != data_line.end();
       ++row_str) {
    std::vector<std::string> item_vec;
    boost::split(item_vec, *row_str, boost::is_any_of(","),
                 boost::token_compress_on);
...
bool
SQLCase::CreateRowsFromYamlNode(const YAML::Node &node,
...
bool
SQLCase::CreateSQLCasesFromYaml(const std::string &cases_dir,
                                const std::string &yaml_path,
                                std::vector<SQLCase> &sql_cases,
...
std::string FindFesqlDirPath() {
  ...
  if (find_fesql_dir) {
    DLOG(INFO) << "Fesql Dir Path is : " << fesql_path.string() << std::endl;
