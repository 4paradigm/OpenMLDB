...
void CreateAdvanceTablet(::rtidb::tablet::TabletImpl& tablet,
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        ASSERT_EQ(0, response.code());
        {
            ...
            MockClosure closure;
            tablet.Get(NULL, &request, &response, &closure);
            if (time <= expire_time_ts1 && ttl_type == ::rtidb::api::TTLType::kAbsOrLat) {
...

        {
            ...
            MockClosure closure;
            tablet.Get(NULL, &request, &response, &closure);
            if (time <= expire_time_ts2 && ttl_type == ::rtidb::api::TTLType::kAbsOrLat) {
                ASSERT_EQ(307, response.code());
            } else {
                ++count2;
                ASSERT_EQ(0, response.code());
...
TEST_F(TabletMultiPathTest, Memory_Test_read_write_abs_and_lat) {
    ...
    for (uint32_t i = 20; i < 30; i++) {
        CreateAdvanceTablet(tablet_impl, ::rtidb::api::TTLType::kAbsAndLat, 2000, 500, now - 3000 * (60*1000) - 1000,
            i+1, i % 10, ::rtidb::common::StorageMode::kMemory, 3000, 500);
    }
}
...
TEST_F(TabletMultiPathTest, Memory_Test_read_write_abs_or_lat) {
    ...
    for (uint32_t i = 30; i < 40; i++) {
        CreateAdvanceTablet(tablet_impl, ::rtidb::api::TTLType::kAbsOrLat, 2000, 500, now - 3000 * (60*1000) - 1000,
            i+1, i % 10, ::rtidb::common::StorageMode::kMemory, 1000, 500);
    }
}
