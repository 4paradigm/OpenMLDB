...
int FileSender::WriteData(const std::string& file_name, const char* buffer, size_t len, uint64_t block_id) {
	if (buffer == NULL) {
        return -1;
    }
    ...
    if (limit_time_ > time_used && len > FLAGS_stream_block_size / 2) {
        ...
        PDLOG(DEBUG, "sleep %lu us, limit_time %lu time_used %lu", sleep_time, limit_time_, time_used);
...
int FileSender::SendFile(const std::string& file_name, const std::string& full_path) {
	...
	do {
		if (try_times < FLAGS_send_file_max_try) {
            ...
            PDLOG(INFO, "retry to send file %s to %s. total size[%lu]", full_path.c_str(), endpoint_.c_str(), file_size);
        }
        try_times--;
		if (SendFileInternal(file_name, full_path, file_size) < 0) {
			continue;
		}
...
int FileSender::SendFileInternal(const std::string& file_name, const std::string& full_path, uint64_t file_size) {
	...
	do {
		if (block_count == 0 && WriteData(file_name, buffer, 0, block_count) < 0) {
		...
		if (len < FLAGS_stream_block_size) {
			if (feof(file)) {
				if (len > 0) {
					ret = WriteData(file_name, buffer, len, block_count);
				}
				break;
			}
		...
		if (WriteData(file_name, buffer, len, block_count) < 0) {
