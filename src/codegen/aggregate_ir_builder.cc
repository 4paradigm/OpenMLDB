    ...
    switch (expr->expr_type_) {
        case node::kExprCall: {
            ...
            if (iter == agg_col_infos_.end()) {
                agg_col_infos_[col_key] = AggColumnInfo(
                    col, node_type, slice_idx, offset);
            }
...
class StatisticalAggGenerator {
 public:
    ...
    ::llvm::Value* GenSumInitState(::llvm::IRBuilder<>* builder) {
        ...
        ::llvm::Type* llvm_ty = AggregateIRBuilder::GetOutputLLVMType(
            llvm_ctx, "sum", col_type_);
        ::llvm::Value* accum = builder->CreateAlloca(llvm_ty);
        if (llvm_ty->isIntegerTy()) {
            builder->CreateStore(
                ::llvm::ConstantInt::get(llvm_ty, 0, true), accum);
        } else {
            builder->CreateStore(
                ::llvm::ConstantFP::get(llvm_ty, 0.0), accum);
        }
        return accum;
    }
    ...
    ::llvm::Value* GenCountInitState(::llvm::IRBuilder<>* builder) {
        ...
        builder->CreateStore(
            ::llvm::ConstantInt::get(int64_ty, 0, true), cnt);
        return cnt;
    }
    ...
    ::llvm::Value* GenMinInitState(::llvm::IRBuilder<>* builder) {
        ...
        if (llvm_ty == ::llvm::Type::getInt16Ty(llvm_ctx)) {
            ...
            min = ::llvm::ConstantFP::get(
                llvm_ty, std::numeric_limits<float>::max());
        } else {
            min = ::llvm::ConstantFP::get(
                llvm_ty, std::numeric_limits<double>::max());
        }
    ...
    ::llvm::Value* GenMaxInitState(::llvm::IRBuilder<>* builder) {
        ...
        if (llvm_ty == ::llvm::Type::getInt16Ty(llvm_ctx)) {
            ...
            max = ::llvm::ConstantFP::get(
                llvm_ty, std::numeric_limits<float>::min());
        } else {
            max = ::llvm::ConstantFP::get(
                llvm_ty, std::numeric_limits<double>::min());
        }
    ...
    void GenMinUpdate(size_t i, ::llvm::Value* input,
        ...
        ::llvm::FunctionCallee callee = module->getOrInsertFunction(
            std::string("min_") + DataTypeName(col_type_),
            min_ty, min_ty, min_ty);
    ...
    void GenMaxUpdate(size_t i, ::llvm::Value* input,
        ...
        ::llvm::FunctionCallee callee = module->getOrInsertFunction(
            std::string("max_") + DataTypeName(col_type_),
            max_ty, max_ty, max_ty);
    ...
    void RegisterSum(size_t pos, size_t out_idx) {
        sum_idxs_[pos] = out_idx;
    }

    void RegisterAvg(size_t pos, size_t out_idx) {
        avg_idxs_[pos] = out_idx;
    }
    ...
    const std::vector<std::string>& GetColKeys() const {
        return col_keys_;
    }
...
llvm::Type* AggregateIRBuilder::GetOutputLLVMType(
    ...
    switch (node_type) {
        case ::fesql::node::kInt16:
            llvm_ty = ::llvm::Type::getInt16Ty(llvm_ctx); break;
        case ::fesql::node::kInt32:
            llvm_ty = ::llvm::Type::getInt32Ty(llvm_ctx); break;
        case ::fesql::node::kInt64:
            llvm_ty = ::llvm::Type::getInt64Ty(llvm_ctx); break;
        case ::fesql::node::kFloat:
            llvm_ty = ::llvm::Type::getFloatTy(llvm_ctx); break;
        case ::fesql::node::kDouble:
            llvm_ty = ::llvm::Type::getDoubleTy(llvm_ctx); break;
        default: {
...
size_t GetTypeByteSize(node::DataType dtype) {
    switch (dtype) {
        case ::fesql::node::kInt16: return 2;
        case ::fesql::node::kInt32: return 4;
        case ::fesql::node::kInt64: return 8;
        case ::fesql::node::kFloat: return 4;
        case ::fesql::node::kDouble: return 8;
        default: return 0;
    }
}
...
bool AggregateIRBuilder::BuildMulti(
    ...
    std::string fn_name = base_funcname + "_multi_column_agg__";
