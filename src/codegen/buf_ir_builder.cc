...
bool BufIRBuilder::BuildGetField(const std::string& name,
    ...
    ::llvm::Value* llvm_offset;
    ::llvm::Type* llvm_type;

    ::fesql::type::Type fe_type;
    uint32_t offset_int;

    if (GetFieldOffset(name, offset_int, fe_type)) {
        ...
        ::llvm::ConstantInt* llvm_offset = builder.getInt32(offset_int);
        return BuildLoadOffset(builder, row_ptr, llvm_offset, llvm_type,
                               output);
    }
    return false;
}
