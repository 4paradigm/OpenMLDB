    ...
    for (; it != endpoints.end(); ++it) {
        ...
        if (tit == blob_servers_.end()) {
            ...
            if (FLAGS_use_name) {
                ...
                blob->client_ = std::make_shared<BsClient>(FLAGS_use_rdma, *it, real_ep, true);
                ...
                blob->client_ = std::make_shared<BsClient>(FLAGS_use_rdma, *it, "", true);
            }
            ...
            if (tit->second->state_ != TabletState::kTabletHealthy) {
                if (FLAGS_use_name) {
                    ...
                    tit->second->client_ = std::make_shared<BsClient>(FLAGS_use_rdma, tit->first, r_it->second, true);
    ...
    if (sdk_endpoint != "null") {
        ...
        for (auto it = sdk_endpoint_map_.begin(); it != sdk_endpoint_map_.end(); ++it) {
            if (it->second == sdk_endpoint) {
                response->set_code(::rtidb::base::ReturnCode::kSdkEndpointDuplicate);
