  ...
  if (nullptr != index_item_list && 0 != index_item_list->GetSize()) {
    ...
    while (nullptr != cur && nullptr != cur->node_ptr_) {
      switch (cur->node_ptr_->GetType()) {
        ...
        case kIndexTs:
          node_ptr->SetTs(
              dynamic_cast<IndexTsNode *>(cur->node_ptr_)->GetColumnName());
          break;
        case kIndexVersion:
          node_ptr->SetVersion(dynamic_cast<IndexVersionNode *>(cur->node_ptr_)
                                   ->GetColumnName());

          node_ptr->SetVersionCount(
              dynamic_cast<IndexVersionNode *>(cur->node_ptr_)->GetCount());
