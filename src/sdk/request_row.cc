...
static const std::unordered_map<::fesql::sdk::DataType, uint8_t> SDK_TYPE_SIZE_MAP = {
    {::fesql::sdk::kTypeBool, sizeof(bool)},
...
RequestRow::RequestRow(
        const fesql::sdk::Schema* schema):schema_(schema),
    cnt_(0), size_(0), str_field_cnt_(0), str_addr_length_(0),
    str_field_start_offset_(0), str_offset_(0), offset_vec_(),
    val_(), buf_(NULL) {
...
bool RequestRow::Check(fesql::sdk::DataType type) {
    ...
    if (type != ::fesql::sdk::kTypeString) {
        auto iter = SDK_TYPE_SIZE_MAP.find(type);
        if (iter == SDK_TYPE_SIZE_MAP.end()) {
            LOG(WARNING) << fesql::sdk::DataTypeName(type) << " is not supported";
            return false;
        }
    }
    return true;
