...
bool ResultSetSQL::Init() {
    ...
    std::unique_ptr<::fesql::sdk::RowIOBufView> row_view(
        new ::fesql::sdk::RowIOBufView(internal_schema));
    result_set_base_ = new ResultSetBase(cntl_, response_->count(),
            response_->byte_size(), std::move(row_view), internal_schema);
    return true;
}
