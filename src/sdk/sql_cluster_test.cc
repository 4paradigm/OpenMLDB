...
TEST_F(SQLClusterTest, execute_DDL_parse) {
    ...
    auto sql = "SELECT\n" + "  behaviourTable.itemId as itemId,\n" + "  behaviourTable.ip as ip,\n" +
               ...
               "  LAST JOIN feedbackTable ON feedbackTable.itemId = behaviourTable.itemId\n" +
    ...
    table_map.insert(std::make_pair("behaviourTable", table_a_coluExecuteDDLParsemn_list));
    ...
    table_map.insert(std::make_pair("adinfo", table_b_column_list));
    ...
    table_map.insert(std::make_pair("feedbackTable", table_c_column_list));

    std::vector<std::string> ddl_list = router->ExecuteDDLParse(sql, table_map);
    ASSERT_EQ(3, ddl_list.size());
    ASSERT_STREQ("CREATE TABLE IF NOT EXISTS behaviourTable(\n"
        "\titemId string,\n"
        "\treqId string,\n"
        "\ttags string,\n"
        "\tinstanceKey string,\n"
