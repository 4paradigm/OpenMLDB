...
class SQLExecutor(BaseExecutor):
    ...
    def execute(self):
        ...
        if sqls!=None and len(sqls)>0:
            for sql in sqls:
                log.info("sql:"+sql)
                sql = fesql_util.formatSql(sql,self.tableNames)
                fesqlResult = fesql_util.sql(self.executor,self.dbName,sql)
    ...
    def tearDown(self):
        if self.tableNames:
            for tableName in self.tableNames:
                ...
                fesql_util.ddl(self.executor,self.dbName,dropSql)
...
class RequestQuerySQLExecutor(SQLExecutor):
    ...
    def prepare(self):
        ...
        if createDB==None or createDB :
            ...
            log.info("create db:"+self.dbName+","+str(dbOk))
        inputs = self.fesqlCase.get('inputs')
        res,self.tableNames = fesql_util.createAndInsert(self.executor,self.dbName,inputs,True)
        if not res.ok:
    ...
    def execute(self):
        ...
        if sql!=None and len(sql)>0:
            ...
            log.info("sql:" + sql)
            sql = fesql_util.formatSql(sql, self.tableNames)
            fesqlResult = fesql_util.sqlRequestMode(self.executor, self.dbName, sql,inputs[0])
        return fesqlResult
