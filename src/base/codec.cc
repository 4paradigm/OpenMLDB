...
static const std::unordered_map<::rtidb::common::DataType, uint8_t> TYPE_SIZE_MAP = {
    {::rtidb::common::kBool, sizeof(bool)},
    {::rtidb::common::kInt16, sizeof(int16_t)},
    {::rtidb::common::kInt32, sizeof(int32_t)},
    {::rtidb::common::kFloat, sizeof(float)},
    {::rtidb::common::kInt64, sizeof(int64_t)},
    {::rtidb::common::kTimestamp, sizeof(int64_t)},
    {::rtidb::common::kDouble, sizeof(double)}};
...
RowBuilder::RowBuilder(const Schema& schema)
    ...
    for (int idx = 0; idx < schema.size(); idx++) {
        const ::rtidb::common::ColumnDef& column = schema.Get(idx);
        if (column.type() == ::rtidb::common::kVarchar) {
...
bool RowBuilder::Check(::rtidb::common::DataType type) {
    if ((int32_t)cnt_ >= schema_.size()) {
        return false;
    }
    const ::rtidb::common::ColumnDef& column = schema_.Get(cnt_);
    if (column.type() != type) {
        return false;
    }
    if (column.type() != ::rtidb::common::kVarchar) {
...
bool RowBuilder::AppendBool(bool val) {
    if (!Check(::rtidb::common::kBool)) return false;
...
bool RowBuilder::AppendInt32(int32_t val) {
    if (!Check(::rtidb::common::kInt32)) return false;
    int8_t* ptr = buf_ + offset_vec_[cnt_];
...
bool RowBuilder::AppendInt16(int16_t val) {
    if (!Check(::rtidb::common::kInt16)) return false;
    int8_t* ptr = buf_ + offset_vec_[cnt_];
...
bool RowBuilder::AppendTimestamp(int64_t val) {
    if (!Check(::rtidb::common::kTimestamp)) return false;
    int8_t* ptr = buf_ + offset_vec_[cnt_];
...
bool RowBuilder::AppendInt64(int64_t val) {
    if (!Check(::rtidb::common::kInt64)) return false;
    int8_t* ptr = buf_ + offset_vec_[cnt_];
    *(reinterpret_cast<int64_t*>(ptr)) = val;
    cnt_++;
    return true;
...
bool RowBuilder::AppendFloat(float val) {
    if (!Check(::rtidb::common::kFloat)) return false;
    int8_t* ptr = buf_ + offset_vec_[cnt_];
...
bool RowBuilder::AppendDouble(double val) {
    if (!Check(::rtidb::common::kDouble)) return false;
    int8_t* ptr = buf_ + offset_vec_[cnt_];
...
bool RowBuilder::AppendString(const char* val, uint32_t length) {
    if (val == NULL || !Check(::rtidb::common::kVarchar)) return false;
...
bool RowView::Init() {
    ...
    for (int idx = 0; idx < schema_.size(); idx++) {
        const ::rtidb::common::ColumnDef& column = schema_.Get(idx);
        if (column.type() == ::rtidb::common::kVarchar) {
...
bool RowView::CheckValid(uint32_t idx, ::rtidb::common::DataType type) {
...
int32_t RowView::GetBool(uint32_t idx, bool* val) {
    ...
    if (!CheckValid(idx, ::rtidb::common::kBool)) {
        return -1;
    }
    if (IsNULL(row_, idx)) {
        return 1;
...
int32_t RowView::GetInt32(uint32_t idx, int32_t* val) {
    ...
    if (!CheckValid(idx, ::rtidb::common::kInt32)) {
        return -1;
    }
    if (IsNULL(row_, idx)) {
        return 1;
...
int32_t RowView::GetTimestamp(uint32_t idx, int64_t* val) {
    ...
    if (!CheckValid(idx, ::rtidb::common::kTimestamp)) {
        return -1;
    }
    if (IsNULL(row_, idx)) {
        return 1;
...
int32_t RowView::GetInt64(uint32_t idx, int64_t* val) {
    ...
    if (!CheckValid(idx, ::rtidb::common::kInt64)) {
        return -1;
    }
    if (IsNULL(row_, idx)) {
        return 1;
...
int32_t RowView::GetInt16(uint32_t idx, int16_t* val) {
    ...
    if (!CheckValid(idx, ::rtidb::common::kInt16)) {
        return -1;
    }
    if (IsNULL(row_, idx)) {
        return 1;
...
int32_t RowView::GetFloat(uint32_t idx, float* val) {
    ...
    if (!CheckValid(idx, ::rtidb::common::kFloat)) {
        return -1;
    }
    if (IsNULL(row_, idx)) {
        return 1;
...
int32_t RowView::GetDouble(uint32_t idx, double* val) {
    ...
    if (!CheckValid(idx, ::rtidb::common::kDouble)) {
        return -1;
    }
    if (IsNULL(row_, idx)) {
        return 1;
...
int32_t RowView::GetInteger(const int8_t* row, uint32_t idx,
    ...
    int32_t ret = 0;
    switch (type) {
        case ::rtidb::common::kInt16: {
            int16_t tmp_val = 0;
            ret = GetValue(row, idx, type, &tmp_val);
            if (ret == 0) *val = tmp_val;
            break;
        }
        case ::rtidb::common::kInt32: {
            int32_t tmp_val = 0;
            GetValue(row, idx, type, &tmp_val);
            if (ret == 0) *val = tmp_val;
            break;
        }
        case ::rtidb::common::kTimestamp:
        case ::rtidb::common::kInt64: {
            int64_t tmp_val = 0;
            GetValue(row, idx, type, &tmp_val);
            if (ret == 0) *val = tmp_val;
            break;
...
int32_t RowView::GetValue(const int8_t* row, uint32_t idx,
                          ::rtidb::common::DataType type, void* val) {
    if (schema_.size() == 0 || row == NULL) {
        return -1;
    }
    if ((int32_t)idx >= schema_.size()) {
...
int32_t RowView::GetString(uint32_t idx, char** val, uint32_t* length) {
    ...
    if (!CheckValid(idx, ::rtidb::common::kVarchar)) {
        return -1;
    }
    if (IsNULL(row_, idx)) {
        return 1;
