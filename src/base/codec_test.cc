...
TEST_F(CodecTest, EncodeRows_empty) {
    std::vector<std::pair<uint64_t, std::unique_ptr<::rtidb::base::Slice>>>
        data;
    std::string pairs;
...
TEST_F(CodecTest, EncodeRows) {
    ...
    data.push_back(std::make_pair(
        1, std::unique_ptr<::rtidb::base::Slice>(
               new ::rtidb::base::Slice(test1.c_str(), test1.length()))));
    data.push_back(std::make_pair(
        2, std::unique_ptr<::rtidb::base::Slice>(
               new ::rtidb::base::Slice(test2.c_str(), test2.length()))));
    data.push_back(std::make_pair(
        3, std::unique_ptr<::rtidb::base::Slice>(
               new ::rtidb::base::Slice(empty.c_str(), empty.length()))));
    std::string pairs;
