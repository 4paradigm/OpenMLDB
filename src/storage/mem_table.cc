...
bool MemTable::Init() {
    ...
    for (uint32_t i = 0; i < idx_cnt; i++) {
        ...
        if (!index_def || !index_def->IsReady()) {
            PDLOG(WARNING, "init failed, index %u is not exist. tid %u pid %u", i, id_, pid_);
            continue;
        }
    ...
    PDLOG(INFO,
          ...
          "%d",
          name_.c_str(), id_, pid_, seg_cnt_, abs_ttl_.load() / (60 * 1000), lat_ttl_.load());
    return true;
}
...
bool MemTable::Put(uint64_t time, const std::string& value, const Dimensions& dimensions) {
    DataBlock* block = new DataBlock(dimensions.size(), value.c_str(), value.length());
...
bool MemTable::Put(const Dimensions& dimensions, const TSDimensions& ts_dimemsions, const std::string& value) {
    ...
    for (auto iter = dimensions.begin(); iter != dimensions.end(); iter++) {
        ...
        if (!index_def) {
            PDLOG(WARNING, "can not found dimension idx %u. tid %u pid %u", iter->idx(), id_, pid_);
            return false;
        ...
        if (!has_ts) {
            PDLOG(WARNING, "can not found ts in dimension idx %u. tid %u pid %u", iter->idx(), id_, pid_);
            return false;
        }
    }
...
void MemTable::SchedGc() {
    ...
    PDLOG(INFO,
          ...
          "%lu lat_ttl %lu",
          name_.c_str(), id_, pid_, ::rtidb::api::TTLType_Name(ttl_type_).c_str(), abs_ttl_for_log, lat_ttl_for_log);
    uint64_t cur_time = ::baidu::common::timer::get_micros() / 1000;
    uint64_t time = 0;
    if (abs_ttl_.load(std::memory_order_relaxed) != 0) {
        time = cur_time + time_offset_.load(std::memory_order_relaxed) - ttl_offset_ -
               abs_ttl_.load(std::memory_order_relaxed);
    }
        ...
        for (auto ts_idx : ts_vec) {
            ...
            if (ts_idx < abs_ttl_vec_.size()) {
                if (abs_ttl_vec_[ts_idx]->load(std::memory_order_relaxed) == 0) {
                    ...
                    expire_time = cur_time + time_offset_.load(std::memory_order_relaxed) - ttl_offset_ -
        ...
        for (uint32_t j = 0; j < seg_cnt_; j++) {
            ...
            switch (ttl_type_) {
                ...
                case ::rtidb::api::TTLType::kLatestTime:
                    if (cur_ttl_map.empty()) {
                        segment->Gc4Head(lat_ttl_.load(std::memory_order_relaxed), gc_idx_cnt, gc_record_cnt,
                                         gc_record_byte_size);
                    } else {
                ...
                case ::rtidb::api::TTLType::kAbsAndLat:
                    if (cur_ttl_map.empty()) {
                        segment->Gc4TTLAndHead(time, lat_ttl_.load(std::memory_order_relaxed), gc_idx_cnt,
                ...
                case ::rtidb::api::TTLType::kAbsOrLat:
                    if (cur_ttl_map.empty()) {
                        segment->Gc4TTLOrHead(time, lat_ttl_.load(std::memory_order_relaxed), gc_idx_cnt, gc_record_cnt,
                                              gc_record_byte_size);
                    } else {
...
uint64_t MemTable::GetExpireTime(uint64_t ttl) {
    if (!enable_gc_.load(std::memory_order_relaxed) || ttl == 0 || ttl_type_ == ::rtidb::api::TTLType::kLatestTime) {
        return 0;
    }
    uint64_t cur_time = ::baidu::common::timer::get_micros() / 1000;
...
int MemTable::GetCount(uint32_t index, uint32_t ts_idx, const std::string& pk, uint64_t& count) {
    ...
    if (std::find(ts_vec.begin(), ts_vec.end(), ts_idx) == ts_vec.end()) {
        PDLOG(WARNING, "ts index %u is not member of index %u, tid %u pid %u", ts_idx, index, id_, pid_);
        return -1;
    }
    uint32_t seg_idx = 0;
    if (seg_cnt_ > 1) {
...
::fesql::vm::WindowIterator* MemTable::NewWindowIterator(uint32_t index) {
    ...
    return new MemTableKeyIterator(segments_[index], seg_cnt_, ttl_type_, expire_time, expire_cnt, 0);
}
...
::fesql::vm::WindowIterator* MemTable::NewWindowIterator(uint32_t index, uint32_t ts_index) {
    ...
    return new MemTableKeyIterator(segments_[index], seg_cnt_, ttl_type_, expire_time, expire_cnt, ts_index);
}
...
TableIterator* MemTable::NewTraverseIterator(uint32_t index, uint32_t ts_index) {
    ...
    const std::vector<uint32_t> ts_vec = index_def->GetTsColumn();
    if (std::find(ts_vec.begin(), ts_vec.end(), ts_index) == ts_vec.end()) {
        PDLOG(WARNING, "ts index %u is not member of index %u, tid %u pid %u", ts_index, index, id_, pid_);
        return NULL;
    }
    uint64_t expire_time = 0;
    uint64_t expire_cnt = 0;
...
MemTableKeyIterator::MemTableKeyIterator(Segment** segments, uint32_t seg_cnt, ::rtidb::api::TTLType ttl_type,
...
MemTableTraverseIterator::MemTableTraverseIterator(Segment** segments, uint32_t seg_cnt, ::rtidb::api::TTLType ttl_type,
...
bool MemTableTraverseIterator::IsExpired() {
    ...
    if (ttl_type_ == ::rtidb::api::TTLType::kLatestTime) {
        ...
        return ((it_->GetKey() <= expire_value_.abs_ttl) && (expire_value_.abs_ttl != 0)) ||
               ((record_idx_ > expire_value_.lat_ttl) && (expire_value_.lat_ttl != 0));
    }
}
