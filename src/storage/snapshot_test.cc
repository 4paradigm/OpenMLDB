...
TEST_F(SnapshotTest, Recover_binlog_and_snapshot) {
    ...
    std::shared_ptr<MemTable> table = std::make_shared<MemTable>("test", 4, 3, 8, mapping, 0, ::rtidb::common::TTLType::kAbsoluteTime);
    table->Init();
...
TEST_F(SnapshotTest, Recover_only_binlog_multi) {
    ...
    std::shared_ptr<MemTable> table = std::make_shared<MemTable>("test", 4, 4, 8, mapping, 0, ::rtidb::common::TTLType::kAbsoluteTime);
    table->Init();
...
TEST_F(SnapshotTest, Recover_only_binlog) {
    ...
    std::map<std::string, uint32_t> mapping;
    mapping.insert(std::make_pair("idx0", 0));
    std::shared_ptr<MemTable> table = std::make_shared<MemTable>("test", 3, 3, 8, mapping, 0, ::rtidb::common::TTLType::kAbsoluteTime);
    table->Init();
...
TEST_F(SnapshotTest, Recover_only_snapshot_multi) {
    ...
    mapping.insert(std::make_pair("card", 0));
    mapping.insert(std::make_pair("merchant", 1));
    std::shared_ptr<MemTable> table = std::make_shared<MemTable>("test", 3, 2, 8, mapping, 0, ::rtidb::common::TTLType::kAbsoluteTime);
    table->Init();
    LogParts* log_part = new LogParts(12, 4, scmp);
...
TEST_F(SnapshotTest, Recover_only_snapshot) {
    ...
    std::shared_ptr<MemTable> table = std::make_shared<MemTable>("test", 2, 2, 8, mapping, 0, ::rtidb::common::TTLType::kAbsoluteTime);
    table->Init();
    LogParts* log_part = new LogParts(12, 4, scmp);
...
TEST_F(SnapshotTest, MakeSnapshot) {
    ...
    std::map<std::string, uint32_t> mapping;
    mapping.insert(std::make_pair("idx0", 0));
    std::shared_ptr<MemTable> table = std::make_shared<MemTable>("tx_log", 1, 1, 8, mapping, 2, ::rtidb::common::TTLType::kAbsoluteTime);
    table->Init();
    uint64_t offset = 0;
    uint32_t binlog_index = 0;
...
TEST_F(SnapshotTest, MakeSnapshotLatest) {
    ...
    std::map<std::string, uint32_t> mapping;
    mapping.insert(std::make_pair("idx0", 0));
    std::shared_ptr<MemTable> table = std::make_shared<MemTable>("tx_log", 5, 1, 8, mapping, 4, ::rtidb::common::TTLType::kLatestTime);
    table->Init();
    uint64_t offset = 0;
    uint32_t binlog_index = 0;
...
TEST_F(SnapshotTest, Recover_empty_binlog) {
    ...
    std::map<std::string, uint32_t> mapping;
    mapping.insert(std::make_pair("idx0", 0));
    std::shared_ptr<MemTable> table = std::make_shared<MemTable>("test", tid, 0, 8, mapping, 0, ::rtidb::common::TTLType::kAbsoluteTime);
    table->Init();
