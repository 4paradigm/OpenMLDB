...
void Segment::FreeList(::rtidb::base::Node<uint64_t, DataBlock*>* node,
    ...
    while (node != NULL) {
        ...
        PDLOG(DEBUG, "delete key %lld with height %u", tmp->GetKey(),
              tmp->Height());
...
void Segment::FreeEntry(::rtidb::base::Node<Slice, void*>* entry_node,
                        uint64_t& gc_idx_cnt, uint64_t& gc_record_cnt,
                        uint64_t& gc_record_byte_size) {
    if (entry_node == NULL) {
        return;
    ...
    if (ts_cnt_ > 1) {
        ...
        for (uint32_t i = 0; i < ts_cnt_; i++) {
            ...
            if (it->Valid()) {
                ...
                ::rtidb::base::Node<uint64_t, DataBlock*>* data_node =
...
void Segment::GcEntryFreeList(uint64_t version, uint64_t& gc_idx_cnt,
                              ...
                              uint64_t& gc_record_byte_size) {
    ::rtidb::base::Node<uint64_t, ::rtidb::base::Node<Slice, void*>*>* node =
        NULL;
    {
        std::lock_guard<std::mutex> lock(gc_mu_);
    ...
    while (node != NULL) {
        ...
        delete entry_node;
        ::rtidb::base::Node<uint64_t, ::rtidb::base::Node<Slice, void*>*>* tmp =
            node;
        node = node->GetNextNoBarrier(0);
        delete tmp;
        pk_cnt_.fetch_sub(1, std::memory_order_relaxed);
...
void Segment::Gc4Head(uint64_t keep_cnt, uint64_t& gc_idx_cnt,
    ...
    PDLOG(DEBUG, "[Gc4Head] segment gc keep cnt %lu consumed %lld, count %lld",
...
void Segment::Gc4TTL(const uint64_t time, uint64_t& gc_idx_cnt,
    ...
    PDLOG(DEBUG, "[Gc4TTL] segment gc with key %lld ,consumed %lld, count %lld",
