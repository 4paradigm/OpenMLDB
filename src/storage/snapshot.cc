...
namespace rtidb {
    namespace storage {
        ...
        int Snapshot::MakeSnapshot(std::shared_ptr<Table> table, uint64_t& out_offset) {
            ...
            ::rtidb::log::LogReader log_reader(log_part_, log_path_);
            uint64_t last_offset = log_reader.GetLastRecordEndOffset();
            if (last_offset < offset_ + FLAGS_make_snapshot_threshold_offset) {
                PDLOG(WARNING, "fail to reach the threshold for offset ");
                making_snapshot_.store(false, std::memory_order_release);
                return -1;
            }
            std::string buffer;
