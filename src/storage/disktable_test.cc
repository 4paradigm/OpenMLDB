...
TEST_F(DiskTableTest, Put) {
    ...
    mapping.insert(std::make_pair("idx0|5", 0));
    PDLOG(INFO, "Create new table");
    ...
    rocksdb::Iterator* it = table->NewIterator(raw_key);
    for (int k = 0; k < 10; k++) {
        ASSERT_TRUE(it->Valid());
        std::string key;
        uint64_t ts = 0;
        ParseKeyAndTs(it->key().ToString(), key, ts);
        ASSERT_EQ(key, raw_key);
        ASSERT_EQ(9537 + 9 - k, ts);
        std::string value1 = it->value().ToString();
...
//    ASSERT_EQ(3, table->GetRecordIdxCnt());
    rocksdb::Iterator* it = table->NewIterator(0, "yjdim0");
    ASSERT_TRUE(it->Valid());
    std::string key;
    uint64_t ts = 0;
    ParseKeyAndTs(it->key().ToString(), key, ts);
...
TEST_F(DiskTableTest, Delete) {
    ...
    rocksdb::Iterator* it = table->NewIterator("test5");
    int count = 0;
    while (it->Valid()) {
        uint64_t ts = 0;
        std::string key;
