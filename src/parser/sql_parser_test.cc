...
INSTANTIATE_TEST_CASE_P(
    ...
    testing::Values(
        "insert into t1 values(1, 2, 3.1, \"string\");",
...
TEST_F(SqlParserTest, Parser_Insert_Stmt) {
    ...
    ASSERT_EQ(dynamic_cast<node::ConstNode*>(insert_stmt->values_[0])->GetInt(), 1);
    ASSERT_EQ(dynamic_cast<node::ConstNode*>(insert_stmt->values_[1])->GetDouble(), 2.3);
    ASSERT_EQ(dynamic_cast<node::ConstNode*>(insert_stmt->values_[2])->GetDouble(), 3.1);
    ASSERT_STREQ(dynamic_cast<node::ConstNode*>(insert_stmt->values_[3])->GetStr(), "string");
//
//
