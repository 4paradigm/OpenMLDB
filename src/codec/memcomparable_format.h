...
__attribute__((unused)) static void CopyInteger(const uchar *from, int length, bool is_unsigned, uchar *to) {
...
__attribute__((unused)) static int PackInteger(const void *from, uint32_t length, bool unsigned_flag, void *to) {
...
__attribute__((unused)) static int PackFloat(const void *from, void *to) {
    ...
    if (nr == (float)0.0) {    // NOLINT
        ...
        if (tmp[0] & 128) {
            ...
            uint16_t exp_part = (((uint16_t)tmp[0] << 8) | (uint16_t)tmp[1] | (uint16_t)32768);
 ...
 */
__attribute__((unused)) static int PackString(const void *src,  // The data to encode
...
__attribute__((unused)) static int UnpackInteger(const void *from, uint32_t length, bool unsigned_flag, void *to) {
    ...
    for (uint32_t i = 0, j = length - 1; i < length - 1; ++i, --j) uto[i] = ufrom[j];
    return 0;
}

__attribute__((unused)) static int UnpackFloatingPoint(const void *src, const size_t size, const int exp_digit,
...
__attribute__((unused)) static int UnpackFloat(const void *src, void *dst) {
    ...
    return UnpackFloatingPoint(src, sizeof(float), FLT_EXP_DIG, zero_pattern, (const uchar *)&zero_val, SwapFloatBytes,
                               dst);
}
...
__attribute__((unused)) static int UnpackDouble(const void *src, void *dst) {
    ...
    return UnpackFloatingPoint(src, sizeof(double), DBL_EXP_DIG, zero_pattern, (const uchar *)&zero_val,
                               SwapDoubleBytes, dst);
}
...
*/
__attribute__((unused)) static const uchar *Read(const uint32_t size, const uchar **usrc) {
    const uchar *res;
    res = *usrc;
    *usrc += size;
    return res;
...
*/
__attribute__((unused)) static uint32_t CalcUnpackVariableFormat(uchar flag, bool *done) {
...
/*
   Function of type rdb_index_field_unpack_t
*/
__attribute__((unused)) static int UnpackString(const void *src, void *dst, int32_t *size) {
    if (src == nullptr) {
        return -1;
    }
    const uchar *usrc = (uchar *)src;  // NOLINT
