...
- Add new options `RANGE_BIAS` and `ROWS_BIAS` for `DEPLOYMENT` statement (#3456 @vagetablechicken)
...
### Bug Fixes
- `CREATE TABLE LIKE HIVE` return success if database is not found (#3379 @emo-coder)
- If an error ooccurred when executing `DROP FUNCTION`, it cannot be deleted again. (#3362 @vagetablechicken, #3441 @dl239)
- The result of `SHOW JOBS` are not sorted by `id` (#3371 @emo-coder)
- NameServer will coredump if creating system table failed. (#3432 @dl239)
- `CREATE INDEX` command may fails if the previous `CREATE INDEX` command with the same table have not finished. (#3393 @dl239)
- The result of `SELECT` on the deleted index col is empty (#3426 @dl239)
...
[0.8.2]: https://github.com/4paradigm/OpenMLDB/compare/v0.8.2...v0.8.3
