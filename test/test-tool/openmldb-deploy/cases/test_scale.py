...
class TestScale:
    ...
    def gen_distribution(self, tablets : list, replica_num: int = 3, partition_num : int = 8) -> (Status, str):
    ...
    def test_scalein(self):
        assert len(case_conf.conf["components"]["tablet"]) > 2
        status, distribution = self.gen_distribution(case_conf.conf["components"]["tablet"], 2, 8)
        assert status.OK()
    ...
    def test_scaleout(self):
        assert len(case_conf.conf["components"]["tablet"]) > 2
        status, distribution = self.gen_distribution(case_conf.conf["components"]["tablet"][1:], 2, 8)
        assert status.OK()
        self.cursor.execute("create database if not exists test")
        self.cursor.execute("use test")
        table_name = "table" + str(random.randint(0, 10000))
