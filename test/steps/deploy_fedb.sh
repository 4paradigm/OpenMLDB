...
function deploy_tablet() {
  ...
  local pkg_name=$3
  cp -r "${pkg_name}" "${tablet_name}"
  cd "${tablet_name}" || exit
  sed -i "s#--zk_cluster=.*#--zk_cluster=${ZK_CLUSTER}#" conf/tablet.flags
  sed -i 's@--zk_root_path=.*@--zk_root_path=/openmldb@' conf/tablet.flags
...
deploy_tablet openmldb-tablet-1 "${TABLET1}" "${pkg_name}"
deploy_tablet openmldb-tablet-2 "${TABLET2}" "${pkg_name}"
deploy_tablet openmldb-tablet-3 "${TABLET3}" "${pkg_name}"
# deploy api server
function deploy_api_server() {
  local api_serve_name=$1
  local api_serve_endpoint=$API_SERVER
  local pkg_name=$3
  cp -r "${pkg_name}" "${api_serve_name}"
  cd "${api_serve_name}" || exit
  sed -i "s#--zk_cluster=.*#--zk_cluster=${ZK_CLUSTER}#" conf/apiserver.flags
  sed -i 's@--zk_root_path=.*@--zk_root_path=/openmldb@' conf/apiserver.flags
...
deploy_api_server openmldb-apiserver-1