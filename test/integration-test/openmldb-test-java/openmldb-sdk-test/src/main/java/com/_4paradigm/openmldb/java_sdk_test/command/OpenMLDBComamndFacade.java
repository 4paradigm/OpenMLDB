...
public class OpenMLDBComamndFacade {
    ...
    public static FesqlResult sql(OpenMLDBInfo openMLDBInfo, String dbName, String sql) {
        logger.info("sql:"+sql);
        sql = StringUtils.replace(sql,"\n"," ");
        sql = sql.trim();
        FesqlResult fesqlResult = SqlChainManager.of().sql(openMLDBInfo, dbName, sql);
        logger.info("fesqlResult:"+fesqlResult);
        return fesqlResult;
    }
    public static FesqlResult sqls(OpenMLDBInfo openMLDBInfo, String dbName, List<String> sqls) {
