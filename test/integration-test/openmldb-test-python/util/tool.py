...
class Executor:
    ...
    def MakeSnashot(self, database, name, pid) -> Status:
        ...
        if status.OK() and output.find("ok") != -1:
            if sync and not self.WaitingOP(database, name, pid).OK():
                return Status(-1, "makesnapshot failed")
            return Status()
        return Status(-1, "makesnapshot failed")
    ...
    def WaitingTableOP(self, database, name, partition_num = 8) -> Status:
        for pid in range(partition_num):
            status = self.WaitingOP(database, name, pid)
            if not status.OK():
                return status
        return Status()

    def CheckTableAlive(self, database, name) -> Status:
        status, info = self.GetTableInfo(database, name)
        for item in info:
            if item[5] != "yes":
                return Status(-1, str(item))
        return Status()
