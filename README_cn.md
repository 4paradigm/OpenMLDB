...
#### OpenMLDB 是一个开源的企业级机器学习特征数据平台，依托低门槛数据库开发体验，提供功能闭环的全栈 FeatureOps 解决方案。
...
**低门槛且功能强大的数据库开发体验：** 低门槛的数据库开发体验，全流程基于 SQL 和 CLI 进行特征脚本开发以及部署上线。

**线上线下一致性执行引擎：** 线上线下计算使用底层统一的计算执行引擎，线上线下一致性得到了天然的保证。

**高并发、低延迟：** 线上服务在高吞吐压力下的复杂查询提供几十毫秒量级的延迟，充分满足高并发、低延迟的性能需求。
