...
class TestMakeSnapshotNsClient(TestCaseBase):
    ...
    def test_makesnapshot_pid_notexist(self):
        ...
        self.assertTrue('Fail to makesnapshot. error msg:partition not exist' in rs3)
    ...
    def test_changeleader_and_makesnapshot(self):
        ...
        m = utils.gen_table_metadata(
            ...
            ('table_partition', '"{}"'.format(self.slave1), '"1-3"', 'false'),
        )
        utils.gen_table_metadata_file(m, metadata_path)
        ...
        self.put(self.leader, tid, 1, 'testkey0', self.now(), 'testvalue0')
        ...
        rs4 = self.makesnapshot(self.ns_leader, name, 2, 'ns_client')
        self.start_client(self.leaderpath)
        ...
        self.assertEqual(rs2[(name, tid, '3', self.slave1)], ['follower', '2', '144000', 'yes'])

        self.assertEqual('MakeSnapshot ok' in rs3, True)
        self.assertEqual('Fail to makesnapshot. error msg:tablet is not online' in rs4, True)
        mf = self.get_manifest(self.slave1path, tid, 1)
        self.assertEqual(mf['offset'], '1')
        self.assertTrue(mf['name'])
        self.assertEqual(mf['count'], '1')
