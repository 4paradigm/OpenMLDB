...
if [[ "$ARCH" = "x86_64" ]]; then
  ...
  curl -Lo zetasql.tar.gz "https://github.com/4paradigm/zetasql/releases/download/v${ZETASQL_VERSION}/libzetasql-${ZETASQL_VERSION}-linux-gnu-x86_64-ubuntu.tar.gz" &&
  ...
  curl -Lo thirdparty.tar.gz "https://blog.shouren.me/thirdparty-v0.7.1-linux-gnu-${ARCH}-ubuntu.tar.gz" &&
    echo "downloaded thirdparty.tar.gz version $THIRDPARTY_VERSION for $ARCH"
  ...
  curl -Lo zetasql.tar.gz "https://blog.shouren.me/libzetasql-v0.3.5-linux-gnu-${ARCH}-ubuntu.tar.gz" &&
    echo "downloaded zetasql.tar.gz version $ZETASQL_VERSION for $ARCH"
else
