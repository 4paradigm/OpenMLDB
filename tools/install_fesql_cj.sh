...
cd fesql && mkdir -p build
cd build && cmake -DCMAKE_BUILD_TYPE=${CMAKE_TYPE} -DCMAKE_INSTALL_PREFIX="${RTIDB_THIRDPARTY}" -DTESTING_ENABLE=ON -DCOVERAGE_ENABLE=OFF -DPYSDK_ENABLE=ON -DJAVASDK_ENABLE=ON -DEXPRIRMENT_ENABLE=OFF ..  && make -j5 install
