...
import com._4paradigm.dataimporter.parseParquet.PutTask;
import com._4paradigm.dataimporter.test.CsvExample;
...
public class ParseCsvUtil {
    ...
    public static void putCsv(String path,String tableName, List<ColumnDesc> schemaList){
        ...
        try {
            CsvReader reader = new CsvReader(csvFilePath, separator, Charset.forName(encodingFormat));
            ...
            for (int index = 0; reader.readRecord(); index++) {
                ...
                for (int j = 0; j < reader.getValues().length; j++) {
                    String columnName = schemaList.get(j).getName();
                    switch(columnName){
                        case "int_32":
