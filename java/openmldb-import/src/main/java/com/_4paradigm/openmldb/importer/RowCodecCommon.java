...
//        TYPE_SIZE_MAP.put(DataType.kBlob, 8); // TODO(hw): ?
    ...
    public static int calStrLength(Map<String, Object> row, List<ColumnDesc> schema) {
        int strLength = 0;
        for (int i = 0; i < schema.size(); i++) {
            ColumnDesc columnDesc = schema.get(i);
            Object column = row.get(columnDesc.getName());
            if (columnDesc.getDataType().equals(DataType.kString)) {
                if (!columnDesc.getNotNull() && column == null) {
                    continue;
                } else if (columnDesc.getNotNull()
                        && column == null) {
    ...
    public static int calStrLength(Object[] row, List<ColumnDesc> schema) throws RuntimeException {
        int strLength = 0;
        for (int i = 0; i < schema.size(); i++) {
            ColumnDesc columnDesc = schema.get(i);
            Object column = row[i];
