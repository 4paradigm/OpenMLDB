...
object ConstProjectPlan {
  ...
  def gen(ctx: PlanContext, node: PhysicalConstProjectNode, inputs: Seq[SparkInstance]): SparkInstance = {
    ...
    val selectColList = (0 until columnSourceList.size()).map(i => {
      ...
      columnSource.`type`() match {
        case SourceType.kSourceColumn => {
          throw new IllegalArgumentException(s"Should not handle source column for const proejct node")
        }
    ...
    outputType match {
      ...
      case Type.kInt16 => {
        const_value.GetDataType() match {
          ...
          case _ => throw new IllegalArgumentException(s"FESQL type from ${const_value.GetDataType()} to ${outputType} is not supported")
        }
      }
      case Type.kInt32 => {
        const_value.GetDataType() match {
      ...
      case _ => throw new IllegalArgumentException(s"FESQL type ${outputType} not supported")
    }
  }
