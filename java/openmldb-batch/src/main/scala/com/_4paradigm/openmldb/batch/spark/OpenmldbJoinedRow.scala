...
package com._4paradigm.openmldb.batch.spark
...
/**
 * The custom JoinedRow which fix the issue when updating child UnsafeRow.
 */
class OpenmldbJoinedRow extends InternalRow {
  ...
  def apply(r1: InternalRow, r2: InternalRow): OpenmldbJoinedRow = {
    row1 = r1
    row2 = r2
    this
  }
  ...
  def withLeft(newLeft: InternalRow): OpenmldbJoinedRow = {
    row1 = newLeft
    this
  }

  /** Updates this JoinedRow by updating its right base row.  Returns itself. */
  def withRight(newRight: InternalRow): OpenmldbJoinedRow = {
    row2 = newRight
    this
  }
  ...
  // Overwrite setLong to avoid using `update()` which is not supported in UnsafeRow
  override def setLong(i: Int, value: Long): Unit = {
    if (i < row1.numFields) {
      row1.setLong(i, value)
    } else {
      row2.setLong(i - row1.numFields, value)
    }
  }
