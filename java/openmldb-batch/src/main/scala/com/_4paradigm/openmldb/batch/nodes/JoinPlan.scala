      ...
      if (ctx.getConf.enableJoinWithNativeExpr) {
        ...
        expr.GetExprType() match {
          case ExprType.kExprBinary =>
            ...
            op match {
              case FnOperator.kFnOpAnd =>
                ...
                val leftCondition = leftExpr.GetOp() match {
                  case FnOperator.kFnOpNeq =>
                    val (left, right) = ExpressionUtil.binaryExprToSparkColumns(leftExpr, node, leftDf, rightDf)
                    left.notEqual(right)
                ...
                val rightCondition = rightExpr.GetOp() match {
                  case FnOperator.kFnOpNeq =>
                    val (left, right) = ExpressionUtil.binaryExprToSparkColumns(rightExpr, node, leftDf, rightDf)
                    left.notEqual(right)
                  case FnOperator.kFnOpLt =>
                    val (left, right) = ExpressionUtil.binaryExprToSparkColumns(rightExpr, node, leftDf, rightDf)
                    left < right
                ...
                joinConditions += leftCondition.and(rightCondition)
              case FnOperator.kFnOpOr =>
                ...
                joinConditions += leftCondition.or(rightCondition)

              case FnOperator.kFnOpNot =>
              case FnOperator.kFnOpEq =>
          ...
          case _ => throw new UnsupportedHybridSeException(
            s"Join condition does not support expression type ${expr.GetExprType}, try disabling join native expr opt")
        }
