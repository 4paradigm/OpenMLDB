...
public class DDLEngineTest {
    ...
    public void testDDL(String desc, String schemaPath, String sqlPath, int replicaNumber, int partitionNumber, String expct) {
        ...
        try {
            String ddl = DDLEngine.genDDL(FileUtils.readFileToString(sql, "UTF-8"), FileUtils.readFileToString(file, "UTF-8"), replicaNumber, partitionNumber);
    ...
    public void testDDLAndConfig(String name, File ddlFile, File configFile) throws Exception {
        ...
        String ddl = DDLEngine.genDDL(FileUtils.readFileToString(ddlFile, "UTF-8"), FileUtils.readFileToString(configFile, "UTF-8"), 1, 1);
        String config = DDLEngine.sql2Feconfig(FileUtils.readFileToString(ddlFile, "UTF-8"), FileUtils.readFileToString(configFile, "UTF-8"));
    ...
    @Test
    public void testGenFedbDdl() {
        Map<String, String> nameParquetMap = new HashMap<String, String>();
        String parquetFilePath = DDLEngineTest.class.getClassLoader().getResource("fz/fengdian/action/part-00000-480bd455-bec0-4c9b-a0b0-fbac0efc82a5-c000.snappy.parquet").getPath();
        nameParquetMap.put("t1", parquetFilePath);
        String sql = "SELECT * FROM t1;";

        String expectedDdlSql = "create table `t1`(\n" +
                "`reqId` string,\n" +
                "`eventTime` bigint,\n" +
        ...
        try {
            ...
            Assert.assertEquals(ddlSql, expectedDdlSql);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
