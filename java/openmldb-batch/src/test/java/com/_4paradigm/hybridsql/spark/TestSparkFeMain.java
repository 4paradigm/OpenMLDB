...
public class TestSparkFeMain {
    private static final Logger logger = LoggerFactory.getLogger(TestSparkFeMain.class);
    ...
    public Object[][] getConfig() {
        ...
        return new Object[][] {
                new Object[] {
                        ...
                        "spark.master=local spark.sparkfe.skew.watershed=1 spark.sparkfe.test.print=false spark.sparkfe.test.print.sampleInterval=1 spark.sparkfe.test.print.printContent=false spark.sparkfe.mode=skew spark.sparkfe.skew.level=2 spark.sparkfe.group.partitions=20 spark.sql.shuffle.partitions=8",
                        "spark.sparkfe.mode=normal spark.master=local"
                },
                new Object[] {
                        "风电场景 时间窗口",
                        ...
                        "output/fengdian/skew",
                        "output/fengdian/no-skew",
                        "spark.master=local spark.sparkfe.skew.watershed=1 spark.sparkfe.test.print=false spark.sparkfe.mode=skew spark.sparkfe.skew.level=2 spark.sparkfe.group.partitions=20 spark.sql.shuffle.partitions=8",
                        "spark.sparkfe.mode=normal spark.master=local"
                },
                new Object[] {
                        "风电场景 时间和条数窗口混合",
    ...
    public void testSkewModeResult(String desc, String scriptPath, String jsonPath, String[] checkField, String output1, String output2, String config1, String config2) throws IOException {
        logger.info(desc);
        jsonPath = TestsparkfeMain.class.getClassLoader().getResource(jsonPath).getPath();
        scriptPath = TestsparkfeMain.class.getClassLoader().getResource(scriptPath).getPath();
        String root = TestsparkfeMain.class.getClassLoader().getResource(".").getPath();
        FileUtils.forceMkdir(new File(root + "/" + output1));
        FileUtils.forceMkdir(new File(root + "/" + output2));
        output1 = TestsparkfeMain.class.getClassLoader().getResource(output1).getPath();
        output2 = TestsparkfeMain.class.getClassLoader().getResource(output2).getPath();
