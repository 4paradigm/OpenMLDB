...
object JobInfoManager {
  ...
  def createJobInfo(jobType: String): JobInfo = {
    ...
    val jobInfo = new JobInfo(jobId, jobType, "SUBMITTED", startTime, new Timestamp(0l), "", "", "")
    jobInfo.sync()
    jobInfo
  }
  ...
  def syncJob(job: JobInfo): Unit = {
    val insertSql =
      s"""
         ...
         | (${job.getId}, "${job.getJobType}", "${job.getState}", ${job.getStartTime.getTime}, ${job.getEndTime.getTime}, "${job.getParameter}", "${job.getApplicationId}", "${job.getError}")
         | ;
         |""".stripMargin
