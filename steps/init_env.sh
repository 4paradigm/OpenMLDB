...
if [[ ${HYBRIDSE_SOURCE} = "local" ]]; then
    ...
    if [[ "$OSTYPE" = "darwin"* ]]; then
        curl -SLo hybridse.tar.gz https://github.com/jingchen2222/OpenMLDB/releases/download/hybridse-v0.2.4-test2/hybridse-0.2.4-test2-darwin-x86_64.tar.gz
    elif [[ "$OSTYPE" = "linux-gnu"* ]]; then
        if [[ $ARCH = 'x86_64' ]]; then
            curl -SLo hybridse.tar.gz https://github.com/jingchen2222/OpenMLDB/releases/download/hybridse-v0.2.4-test2/hybridse-0.2.4-test2-linux-x86_64.tar.gz
